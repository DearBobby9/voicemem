...
| **CLI 工具** (`voicemem`) | AI Agent | JSON 输出，可包装为 Codex Skill |
...
- **CLI 工具是一等公民** — `voicemem` CLI 输出 JSON，可被 Codex Skill / Perch Chat / 其他 AI Agent 调用
...
- **Phase 1**: CLI 工具 (`voicemem`) + Codex Skill + FTS5 搜索
...
1. **AGENTS.md** — if architecture, tech stack, or project structure changed
2. **Memory files** (`~/.Codex/projects/.../memory/`) — save stable patterns, decisions, and debugging insights learned during the session

### Plan Mode: Persist After Planning — 死规矩

**列完计划后，必须先更新所有文档，然后才能 ExitPlanMode 让用户接管。** 用户会在 approve 后清空上下文，下一个 Codex 实例没有对话历史。

必须按顺序更新：
1. **AGENTS.md** — 如果计划涉及架构、技术栈、端点、项目结构变化
2. **Memory files** (`~/.Codex/projects/.../memory/`) — 写入计划摘要、当前阶段、决策要点、下一步实现步骤
...
| Codex Skill | P0 (Phase 1) | CLI | Skill wraps CLI |
